...
class Sphere {
	...
	puckRules = function() {
		...
		if (this.x > (bwidth - this.radius) || this.x < this.radius) {
			...
			if (score[0] === 3 || score[1] === 3) {
				...
				setTimeout(function(){window.location.replace((score[0] === 3) ? "/win_comp" : "/lose_comp")}, 2000);
			}
    ...
    movePlayerTwo = function() {
        this.speed_limit = 6
        if(puck.x > center_x - 1) {
            var x_distance = puck.x - this.x, y_distance = puck.y - this.y, totalR = this.radius + puck.radius;
            if(this.velocityX < this.speed_limit){
                if (x_distance > totalR) this.velocityX += this.a;
                else if (x_distance < -1 * totalR) this.velocityX -= this.a;
            } 
            if(this.velocityY < this.speed_limit) {
                if(!(x_distance > this.radius)){
                    if (y_distance > totalR) this.velocityY += this.a;
                    else if (y_distance < -1 * totalR) this.velocityY -= this.a;
                    else this.velocityY = 0;
                } else this.velocityY = 0;
            }
        } else {
            if (this.x > this.x_init + 30) this.velocityX -= this.a;
            else if (this.x < this.x_init - 30) this.velocityX += this.a;
            else this.velocityX = 0;
            if (this.y > this.y_init + 30) this.velocityY -= this.a;
            else if (this.y < this.y_init - 30) this.velocityY += this.a;
            else this.velocityY = 0;
        }
    }
}
...
const playerTwo = new Sphere(bwidth - 120, center_y, player_r, player_mass, 0.3, '#FF8C00', 0, 0, "player")
